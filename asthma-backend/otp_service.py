...
def send_otp_email(email: str, otp: str, purpose: str):
    ...
    methods = []
    # Try explicit STARTTLS on the configured port first (common for Gmail/587)
    methods.append(("starttls", smtp_host, smtp_port))
    # Then try implicit SSL (common on port 465)
    methods.append(("ssl", smtp_host, 465))

    for method_name, host, port in methods:
        for attempt in range(1, 2): # reduced to 1 attempt for speed
            try:
                if method_name == "ssl":
