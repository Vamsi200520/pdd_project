...
@app.delete("/pefr/records/{record_id}")
def delete_pefr_record(
    record_id: int, 
    db: Session = Depends(database.get_db), 
    current_user: models.User = Depends(auth.get_current_user)
):
    record = db.query(models.PEFRRecord).filter(
        models.PEFRRecord.id == record_id,
        models.PEFRRecord.owner_id == current_user.id
    ).first()
    
    if not record:
        raise HTTPException(status_code=404, detail="PEFR record not found")
...
# --- ALERTS ENDPOINTS ---
@app.get("/alerts", response_model=List[schemas.AlertLog])
def get_my_alerts(
    db: Session = Depends(database.get_db), 
    current_user: models.User = Depends(auth.get_current_user)
):
    return db.query(models.AlertLog).filter(models.AlertLog.user_id == current_user.id).order_by(models.AlertLog.timestamp.desc()).all()
...
@app.get("/contacts/emergency", response_model=List[schemas.EmergencyContact])
...
@app.get("/patient/{patient_id}/profile", response_model=schemas.User)
...
@app.get("/patient/{patient_id}/medications", response_model=List[schemas.Medication])
def get_patient_medications(
    ...
    return db.query(models.Medication).filter(models.Medication.owner_id == patient_id).all()
...
def delete_linked_patient(
    ...
    if current_user.role != models.UserRole.DOCTOR:
        raise HTTPException(status_code=403, detail="Only doctors can manage patients.")
...
def prescribe_medication(
    ...
    # Ensure we don't pass duplicate fields that we set explicitly below
    med_data = medication.dict()
    med_data.pop('source', None)
    med_data.pop('prescribed_by', None)
    med_data.pop('start_date', None)
    med_data.pop('taken_status', None)
